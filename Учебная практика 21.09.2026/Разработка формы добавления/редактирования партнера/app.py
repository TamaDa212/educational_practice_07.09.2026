...
from data import UI_RESOURCES, UI_STATIC, load_partner_card
from catalog import load_cards
from navigation import EDIT, LIST, MISSING, PARTNER, Navigator, parse_partner_id
...
def edit_window(partner_id, values=None, errors=None, notice=None):
    ...
    if values is None:
        if parsed is None:
            ...
            partner = load_partner_card(parsed)
            if partner is None:
                return None
...
def page_for(screen, params, window=None):
    ...
    if screen == PARTNER:
        partner = load_partner_card(params.get("partner_id"))
        if partner is None:
        ...
        return render_screen(PARTNER, params, partner=partner, trail=trail)
    if screen == EDIT:
        if window is None:
...
class CRMHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in {"/", "/index.html", "/partners"}:
            html = page_for(*NAVIGATOR.open_list())
            self._send(200, "text/html; charset=utf-8", html.encode("utf-8"))
            return
        ...
        if len(parts) >= 2 and parts[0] == "partner":
            ...
            if len(parts) >= 3 and parts[2] == "edit":
                ...
                html = page_for(*NAVIGATOR.open_partner(partner_id))
            self._send(200, "text/html; charset=utf-8", html.encode("utf-8"))
            return
    ...
    def do_POST(self):
        ...
        partner_id = (fields.get("partner_id") or [""])[0]
        values = values_from_post(fields)
        errors = validate(values)
        NAVIGATOR.open_edit(partner_id or None)
        if errors:
            window = PartnerEditWindow(partner_id, values, errors)
        else:
            window = PartnerEditWindow(
                partner_id, values, notice="Данные приняты. Типы полей соответствуют ТЗ."
            )
