...
class Navigator:
    """Стек: список → карточка → PartnerEditWindow. ID партнёра не теряется при возврате."""
    ...
    def open_edit(self, partner_id=None):
        # partner_id=None — добавление; иначе тот же ID уходит в hidden-поле формы.
        parsed = parse_partner_id(partner_id)
